...
from nocp.models.contracts import ChatMessage, CompressionMethod, ContextData, ToolRequest, ToolType
...
class ChatBot:
    ...
    def _register_tools(self):
        ...
        async def set_reminder(task: str, time: str) -> dict:
            """Set a reminder."""
            return {"task": task, "time": time, "status": "Reminder set successfully"}

    def _detect_intent(self, message: str) -> dict | None:
        """
        ...
        if any(word in message_lower for word in ["weather", "temperature", "forecast"]):
            ...
            for i, word in enumerate(words):
                if word.lower() in ["in", "at", "for"]:
                    if i + 1 < len(words):
                        ...
                        return {"tool_id": "get_weather", "parameters": {"location": location}}
            ...
            for trigger in ["search for", "find", "look up", "google"]:
                if trigger in message_lower:
                    ...
                    return {"tool_id": "search_web", "parameters": {"query": query}}
            ...
            for trigger in ["calculate", "compute", "what is", "what's"]:
                if trigger in message_lower:
                    ...
                    return {"tool_id": "calculate", "parameters": {"expression": expression}}
            ...
            return {
                ...
                "parameters": {"task": message, "time": "later"},  # Simplified
            }
    ...
    async def chat(self, user_message: str) -> str:
        ...
        if intent:
            ...
            print(
                f"[Tool executed: {tool_result['tool']} in {tool_result['execution_time_ms']:.2f}ms]"
            )
        ...
        if len(messages) > 0:
            context = ContextData(tool_results=[], message_history=messages, max_tokens=100_000)

            optimized = self.context_manager.optimize(context)

            if optimized.method_used != CompressionMethod.NONE:
                print(
                    f"[History compressed: {optimized.original_tokens} → {optimized.optimized_tokens} tokens]"
                )
